...
FString BuildAnimationIntegrationSummaryText(const FWanaSelectedCharacterEnhancementSnapshot* Snapshot)
{
    ...
    return FString::Printf(
        ...
        *WanaWorksUIFormattingUtils::GetAutomaticAnimationIntegrationStatusLabel(Snapshot->AnimationAutomaticIntegrationStatus),
        ...
        Snapshot->RuntimeAnimationAdapterReadiness.IsEmpty() ? (Snapshot->bHasRuntimeAnimationAdapterComponent ? TEXT("Limited") : TEXT("Needs Enhance")) : *Snapshot->RuntimeAnimationAdapterReadiness,
        Snapshot->bRuntimeAnimationAdapterHookReadable ? TEXT("Ready") : TEXT("Needs Enhance"),
        Snapshot->bRuntimeAnimationAdapterReportReadable ? TEXT("Readable") : (Snapshot->RuntimeAnimationAdapterReportPath.IsEmpty() ? TEXT("Not Found") : TEXT("Path Assigned")),
        Snapshot->RuntimeAnimationAdapterRecommendedStrategy.IsEmpty() ? *GetAdapterStrategySummary(*Snapshot) : *Snapshot->RuntimeAnimationAdapterRecommendedStrategy,
        Snapshot->RuntimeAnimationAdapterDetail.IsEmpty() ? TEXT("Runtime adapter will be prepared on the working or finalized subject when Enhance/Build can safely attach it.") : *Snapshot->RuntimeAnimationAdapterDetail,
